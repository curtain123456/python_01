...
# 1.定义用户默认数据: [a1,a2]
user_list = [{'role': 'root', 'account': 'admin', 'password': '123456', 'department': 'dev'},
             {'role': 'admin', 'account': 'dev', 'password': '123456', 'department': 'tester'}]


# 2.定义默认返回结果
# 字典形式  包含两个字段，code和message ,code为0代表成功，为1代表失败
...
def login(username,password):
    ...
    #用户名或密码不匹配或错误的情况
    result.update({'code':'1','message':'请检查你的用户名和密码是否正确'})
    return  result
